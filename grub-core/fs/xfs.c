...
static int grub_xfs_inode_has_large_extents (const struct grub_xfs_inode *inode)
{
...
static grub_uint64_t
grub_xfs_get_extents (struct grub_xfs_inode *inode)
{
  if (grub_xfs_inode_has_large_extents (inode))
    return grub_be_to_cpu64 (inode->di_big_nextents);

  return grub_be_to_cpu32 (inode->nextents);
}
...
grub_xfs_read_block (grub_fshelp_node_t node, grub_disk_addr_t fileblock)
{
  ...
  if (node->inode.format == XFS_INODE_FORMAT_BTREE)
  ...
  else if (node->inode.format == XFS_INODE_FORMAT_EXT)
    {
      ...
      nrec = grub_xfs_get_extents(&node->inode);
